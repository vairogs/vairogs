# Vairogs Functions: Local - Features

This document lists all the functions available in the Vairogs Functions: Local package.
...
try {
    $dirPath = sys_get_temp_dir() . '/vairogs_test';
...
try {
    $nestedPath = sys_get_temp_dir() . '/vairogs_test/nested/subdirectory';
...
$testDir = sys_get_temp_dir() . '/vairogs_rmdir_test';
if (!is_dir($testDir)) {
    mkdir($testDir);
}
...
$tempDir = sys_get_temp_dir() . '/vairogs_temp';
if (!is_dir($tempDir)) {
    mkdir($tempDir);
    file_put_contents($tempDir . '/temp_data.txt', 'Temporary data');
}
$cleanupResult = cleanupTempDirectory($tempDir);
// Output: Successfully cleaned up and removed /tmp/vairogs_temp.
```
